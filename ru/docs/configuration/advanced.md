...
Большинство пользователей могут пропустить эту страницу.

Эти настройки для расширенных конфигураций и настройки уровня сервиса: `daemon`, `api`, `fwmark`, `iproute` и `lists_autoupdate`.
...
|---|---|---|---|
| `pid_file` | string | — | Путь для записи PID-файла |
| `cache_dir` | string | `/var/cache/keen-pbr` | Каталог для кэшированных данных списков |
| `firewall_backend` | string | `"auto"` | Выбор бэкенда firewall: `auto`, `iptables` или `nftables` |
| `strict_enforcement` | boolean | `false` | Строгое применение маршрутизации по умолчанию для интерфейсных outbounds. Когда включено, недостижимый маршрут по умолчанию устанавливается, если шлюз/интерфейс outbound не подтверждается достижимым. Может быть переопределено для каждого outbound. |
| `max_file_size_bytes` | integer | `8388608` (8 MiB) | Максимально допустимый размер в байтах для загруженного содержимого удалённых списков |
| `firewall_verify_max_bytes` | integer | `262144` | Максимумstdout-байт, захваченных за одну команду верификации firewall (`0` = без ограничений) |
