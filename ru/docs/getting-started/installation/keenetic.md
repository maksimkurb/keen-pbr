...
По умолчанию установлен режим `auto`: keen-pbr использует RAW для пересылаемого
IPv4-трафика, если устройство его поддерживает; иначе используется mangle.
Локально созданный трафик и IPv6 в обоих режимах продолжают использовать mangle.

Чтобы узнать выбранный режим, перезапустите сервис и найдите следующие сообщения
в системном журнале устройства:

Строка `raw PREROUTING available; enabling --use-raw-prerouting` означает, что
активен RAW. Сообщение, оканчивающееся на `using mangle PREROUTING`, означает,
что активен mangle.
...
Значение `KEEN_PBR_RAW_PREROUTING="enable"` требует RAW: если возможность RAW
недоступна, сервис завершится с ошибкой вместо перехода на mangle.
