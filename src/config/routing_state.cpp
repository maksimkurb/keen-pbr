...
std::vector<RouteSpec> make_family_closure_routes(
    uint32_t table_id,
    const Outbound& ob,
    const OutboundFamilyAvailabilityFn& family_available,
...
void populate_routing_state(const Config& cfg,
    ...
    // A policy rule must never become visible before every route it can select
    // has been installed. Roll back all state created by this transaction if
    // either phase fails.
    try {
        for (const auto& route : planned_routes) routes.add(route);
        for (const auto& rule : planned_rules) rules.add(rule);
    } catch (...) {
        rules.clear();
        routes.clear();
        throw;
...
bool is_interface_outbound_reachable(const Outbound& outbound, NetlinkManager& netlink) {
    return is_interface_outbound_reachable(outbound,
                                           netlink.dump_routes_in_table(254));
}

bool is_interface_outbound_reachable(
    const Outbound& outbound, const std::vector<DumpedRoute>& routes) {
    if (outbound.type != OutboundType::INTERFACE) {
        return true;
    }
...
bool interface_has_routed_ipv6(const DumpedInterface& interface) {
    return std::any_of(
        ...
        [](const std::string& cidr) {
            ...
            in6_addr parsed{};
            if (inet_pton(AF_INET6, address.c_str(), &parsed) != 1) return false;
            return !IN6_IS_ADDR_UNSPECIFIED(&parsed) &&
                   !IN6_IS_ADDR_LOOPBACK(&parsed) &&
