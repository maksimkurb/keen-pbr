...
func (c *ServerCommand) Run(ctx *AppContext) error {
	log.Infof("Starting keen-pbr API server")
	log.Infof("Config file: %s", ctx.ConfigPath)
	...
	// Create API server
	server := api.NewServer(ctx.ConfigPath, c.bindAddr)
	...
	case sig := <-sigChan:
		...
		shutdownCtx, cancel := context.WithTimeout(context.Background(), 10*syscall.Second)
		defer cancel()
