  ...
  "outbounds": [
    {
      ...
      // Необязательный IPv4-шлюз для interface outbound. Укажите "auto",
      // чтобы выбрать единственный подходящий маршрут по умолчанию с
      // наименьшей метрикой в основной таблице; без поля маршрут будет без шлюза.
      // По умолчанию: null
      "gateway": "10.8.0.1",

      // Необязательный IPv6-шлюз для interface outbound. Укажите "auto",
      // чтобы выбрать единственный подходящий маршрут по умолчанию с
      // наименьшей метрикой в основной таблице; без поля маршрут будет без шлюза.
      // По умолчанию: null
      "gateway6": "2001:db8::1",
    ...
    
    {
      ...
      "tag": "wan",
      "interface": "eth0",
      // Укажите неизменный gateway явно или используйте "auto" для динамического
      // WAN-шлюза, найденного в подходящем маршруте основной таблицы.
      "gateway": "auto"
    },
...

    {
      ...
      // Стратегия: "priority" (по умолчанию) оставляет одного выбранного ребёнка.
      // "balance" только для nftables поровну распределяет новые соединения
      // между пригодными детьми первой здоровой группы с минимальным weight.
      "strategy": "priority",
  ...
  "route": {
    ...
    "rules": [
...

      {
        ...
        // Catch-all только для nftables: IPv4 вне local и connected сетей.
        // Правила default_gateway ставьте последними; для IPv6 добавьте отдельное.
