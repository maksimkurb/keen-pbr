...
```json {filename="config.json"}
{
  ...
  // По умолчанию: пустая строка (используется стандартный брендинг keen-pbr).
  ...
  "daemon": {
    ...
    // Не действует с nftables. Оставьте null или опустите поле для значения по умолчанию (1024).
    ...
    // Не действует с nftables. Оставьте null или опустите поле для значения по умолчанию (65536).
    // Изменение при работающем iptables пересоздаёт owned ipset и очищает
    // изученные dnsmasq адреса.
    "ipset_maxelem": null,
