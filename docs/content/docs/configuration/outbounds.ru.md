...
`icmptest` выбирает кандидатов так же, как `urltest`, но проверяет каждый путь
строгими ICMP Echo request/reply. Для каждого кандидата задаётся отдельный
literal IPv4- или IPv6-адрес в записи группы. Он поддерживает те же стратегии:
`priority` (по умолчанию) и `balance` только для nftables.
...
Старый формат с `outbound_groups[].outbounds` и верхнеуровневым `probes`
при загрузке мигрируется в памяти. Канонический вложенный формат будет записан
при следующем явном сохранении конфигурации; сам запуск файл не перезаписывает.
Смешивать старые и новые поля нельзя.

Попытки выполняются последовательно. После обработки ответа или таймаута демон
ждёт `packet_interval_ms` и только затем отправляет следующий пакет. Worst-case
для кандидата равен
`count * probe_timeout_ms + (count - 1) * packet_interval_ms`; `interval_ms`
должен покрывать все кандидатские проверки и запас 25%.

Ответ засчитывается только при совпадении target, ICMP type, identifier и
sequence. Посторонние и запоздавшие ответы игнорируются до deadline текущей
попытки. Ответ медленнее `max_rtt_ms` считается ошибкой, но отображается в
runtime-счётчиках полученных пакетов.

Сервер разрешает 1–10 пакетов на кандидата, 1–16 уникальных кандидатов, не более
160 пакетов и не более 10 минут worst-case на sweep. Пауза ограничена
100–1000 мс, timeout — 100–5000 мс, а интервал sweep — от одной секунды до
24 часов.
...
При стратегии по умолчанию `priority` правила firewall сохраняют стабильную
mark самого outbound `urltest`. Его policy rule указывает на таблицу выбранного
дочернего outbound, включая уже существующую таблицу outbound типа `table`; при
переключении правила firewall не перестраиваются. `balance` сохраняет эту
стабильную mark для внутренних detour, но обновляет nftables-классификатор после
каждого цикла проверки.

keen-pbr всегда добавляет в сгенерированную fallback-таблицу `urltest`
терминальные маршруты `unreachable` для IPv4 и IPv6. Эта таблица выбирается,
когда нет пригодного кандидата, а терминальное policy rule не позволяет таблице
выбранного кандидата провалиться в обычную системную маршрутизацию.
...
| `strategy` | string | нет (по умолчанию: `"priority"`) | `"priority"` выбирает один дочерний outbound как раньше. `"balance"` только для nftables поровну распределяет новые соединения по пригодным детям первой здоровой группы с минимальным weight. |
...
Группы проверяются по возрастанию `weight` (меньшее значение — более высокий
приоритет); при равном weight сохраняется порядок конфигурации. Внутри первой
здоровой группы `priority` выбирает по задержке. Если все outbounds в группе
нездоровы, оценивается следующий уровень.

| Поле | Тип | Обязательно | Описание |
|---|---|---|---|
| `weight` | integer | нет (по умолчанию: `1`) | Приоритет группы: сначала проверяются меньшие значения. Это не вес отдельного кандидата для балансировки. |
| `outbounds` | array of string | да | Упорядоченный список тегов outbound для попытки |

При `strategy: "balance"` выбор выполняется на соединение, а не на HTTP-запрос.
Мультиплексированные HTTP/2 и QUIC остаются на WAN, выбранном для соединения.
`weight` остаётся только приоритетом группы; дети равны. `tolerance_ms`
относится только к выбору `priority`. При отказе balance-кандидата его conntrack
потоки удаляются; `conntrack_on_switch: "delete"` также удаляет потоки кандидатов
группы при смене скалярного выбора test-group. Если пригодных кандидатов нет,
существующий терминальный fallback test-group блокирует маркированный трафик
вместо утечки в main table.
