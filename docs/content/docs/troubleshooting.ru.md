...
Начните с симптома, который соответствует тому, что вы видите. Каждый раздел начинается с самых простых проверок, затем даёт более продвинутые проверки, если они всё ещё нужны.

## Сервис не запускается

1. Подтвердите, что файл конфигурации существует в обычном месте для вашей платформы.
...
Отрицание для каждой записи в `src_addr` / `dest_addr` не поддерживается. Весь массив должен либо все начинаться с `!`, либо нет.
{{< /callout >}}

Если правила не сопоставляются как ожидается:
- Убедитесь, что `proto` установлен корректно (`"tcp"`, `"udp"` или `"tcp/udp"`)
...
`mask` должна быть точно двумя смежными hex-нибблами. Используйте hex-строки, такие как `"0x00FF0000"` и `"0x00020000"`.
