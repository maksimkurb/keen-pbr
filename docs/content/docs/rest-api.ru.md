...
Эта страница документирует встроенный HTTP REST API.

REST API доступен, когда:
- Установлена полная версия пакета (`keen-pbr`, не `keen-pbr-headless`)
- В конфиге включён `api.enabled: true`
- Флаг `--no-api` не был передан при запуске
...
- Если указан `name`, обновляется только этот список с URL.
- Если `name` опущен, обновляются все списки с URL.
- Если обновлённые данные изменились и влияют на активную маршрутизацию/DNS во время выполнения, keen-pbr перестраивает состояние выполнения, чтобы обновления вступили в силу немедленно.
...
### Статус / Поведение при ошибке

- `200`: Операция обновления завершена.
- `400`: Запрошенный список существует, но не имеет URL.
- `404`: Запрошенный список не найден.
- `409`: Обновление отклонено, потому что существует отложенный черновик или уже выполняется другая операция конфигурации/выполнения.
...
`is_draft` — `true`, когда конфигурация была отложена через `POST /api/config`, но ещё не сохранена на диск.
...
Проверяет живые маршрутизацию и состояние firewall ядра против ожидаемой конфигурации. Проверяет, что цепочка firewall существует, все правила на месте, таблицы маршрутизации заполнены и политические правила на месте.
