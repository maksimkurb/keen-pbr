...
## 3. Установите keen-pbr из репозитория
...
После установки `keen-pbr` включите `dns-override`:

1. Откройте http://my.keenetic.net/a
2. Выполните команды `opkg dns-override` и `system configuration save`
3. Перезагрузите роутер. **Это очень важно: без перезагрузки опция не заработает.**
