...
**Примечания после имплементации:** DNS detour criteria теперь явно помечают
`apply_output`; iptables stable OUTPUT dispatcher и новый nft `output` hook
публикуются атомарно с PREROUTING. Классификатор ограничен exact configured
address:port и TCP/UDP. Для interface/table detours используется internal
mark с unconditional terminal guard, для URLTEST — стабильный normal URLTEST
mark; child mark не используется. OUTPUT chain shape покрыт builder-test.
...
**Примечания после имплементации:** Добавлен versioned length-prefixed JSON
control protocol и Unix `SOCK_STREAM` listener. Socket создаётся по build
option, с mode `0660` и owner `root:keen-pbr`, stale non-socket path
отклоняется, а graceful shutdown удаляет exact socket. `SO_PEERCRED`
ограничивает read/mutate операции root; член `keen-pbr` может выполнять только
list download. IPC client имеет bounded connect/read/write exchange; CLI
`status`, `resolver-config-hash`, `download`/`--reload` и `test-routing`
работают через active daemon state и отклоняют `--config`. Ответы имеют
request_id/status envelope, несовместимая версия выдаёт structured error,
повторная mutation получает `busy`. Framing и реальный Unix client/server
round-trip покрыты unit tests.
...
- Дополнен contract-test `FirewallReconciler`: reconcile неизменного состояния
  подтверждает commit без backend mutations. Проверены регрессии iptables для
  versioned private chain и повторного объявления статического ipset.
- Реальный namespace-suite запускает одинаковые destructive fixtures для
  iptables и nftables; оба backend теперь проходят smoke, URLTEST, формы правил,
  table/interface и multiport fixture. Во время этой проверки исправлена
  совместимость nft JSON conntrack-mark выражений с nftables 1.0.x.
- Выполнены `make`, полный `keen-pbr-tests` (597 test cases, 2374 assertions),
  `clangd-tidy` изменённых firewall/test файлов и `make firewall-it`.
...
- Удаление списка теперь сохраняет route-rule без оставшихся list refs, если
  у него остаётся иной match-критерий (protocol, DSCP, port или address).
  Правила только со списком и DNS rules без list refs по-прежнему удаляются.
- Добавлен Bun unit-test каскадного immutable update: несвязанные правила и
  порядок оставшихся IDs сохраняются, затрагивается только выбранный ID, а
  route-rule с port condition остаётся валидным.
- Проверены targeted Bun tests, `bun run typecheck`, `bun run lint` и
  production `bun run build`. Полный `bun test` выявил предсуществующий
  mismatch в `bulk-selection.test.ts` для `matchingRuleIndexes`, не связанный
  с удалением списков.

---

## US-19. Единый immutable snapshot для resolver stream, expected hash и dnsmasq handshake

**Выполнено: нет.**

**Сложность:** средняя.

**Ожидаемая стабильность:** высокая после unit и integration-покрытия; это
устраняет расхождение между конфигурацией, по которой daemon ожидает TXT hash,
и конфигурацией, фактически переданной dnsmasq.

**Зависимости:** US-01, US-02, US-12, US-13, US-15.

### Фактическое поведение

Expected resolver hash вычисляется из одного пути конфигурации daemon, а IPC
`generate-resolver-config` в `running` может читать другой active snapshot.
Из-за этого daemon способен ожидать hash для правил и domain lists, а dnsmasq
получает неполный stream без `nftset=`, `server=/.../` и
`rebind-domain-ok=` строк. Health при этом может показывать
`runtime_state=running` вместе с `resolver_config_sync_state=stale`.

Реальный OpenWrt случай: active config содержал enabled route rules и DNS rules
для `work`, `cubly_home` и `tailscale`; firewall подтвердил dynamic sets,
однако `keen-pbr generate-resolver-config dnsmasq` выдал только fallback
servers, probe TXT и hash без единой list directive. Повторный restart dnsmasq
в таком состоянии лишь применяет тот же неверный stream.

### Ожидаемое поведение

На границе startup, list-only reload и config apply создать immutable
`ResolverGenerationSnapshot`. Он содержит ровно те данные, из которых строится
resolver desired state:

- config snapshot (route rules, DNS rules, lists, server definitions и IPv6 decision);
- зафиксированный view используемых list/cache sources;
- выбранный firewall/resolver backend;
- canonical expected hash, вычисленный тем же generator pass/директивами;
- generation/transaction identity и timestamp apply.

`DnsmasqGenerator`, expected hash, IPC stream, dnsmasq hook и TXT confirmation
обязаны использовать один и тот же `ResolverGenerationSnapshot`. В `running`
используется только published committed snapshot. Во время `applying` candidate
доступен исключительно внутреннему resolver запросу текущей транзакции; обычные
IPC/HTTP readers продолжают видеть previous committed snapshot или `busy`.

Generator не может считать stream active, если enabled list rule не была
представлена в snapshot или не была обработана: это structured generation error
до первого active byte и fallback вместо частичной конфигурации. После любого
успешного active stream его canonical hash должен в точности совпадать с
expected hash, который подтверждает TXT probe.

Для nftables каждый domain из enabled routed list создаёт `nftset=` directive
для dynamic `kpbr4d_*`/`kpbr6d_*` set. DNS-only list без route rule не создаёт
`nftset=`, но всё равно создаёт соответствующие `server=/.../` и, при
необходимости, `rebind-domain-ok=` directives.

Если observed TXT не совпал с hash того же snapshot после hook timeout,
transaction не публикуется как `running`: выполняется rollback previous
resolver/runtime либо runtime переходит в `broken` при неудачном rollback.

### Критерии приёмки

- Один `ResolverGenerationSnapshot` является единственным источником данных
  для stream, expected hash и TXT confirmation в рамках одной generation.
- Невозможно получить `runtime_state=running`, если resolver stream этого
  committed snapshot не был подтверждён fresh TXT с тем же hash.
- В `running` повторный `generate-resolver-config dnsmasq` выдаёт directives,
  canonical hash которых равен published `resolver_config_hash`.
- Snapshot с enabled route list, содержащим domains, выдаёт `nftset=` для
  каждого допустимого domain; отсутствие всех list directives является error,
  а не valid active config.
- Snapshot с enabled DNS-only list выдаёт domain-scoped `server=`/
  `rebind-domain-ok=` без ложного `nftset=`.
- Route/DNS/list data не могут быть взяты из разных active/candidate snapshots
  в одном stream.
- Обычный IPC CLI не получает candidate во время `applying`; candidate stream
  доступен только запросу, авторизованному текущей apply transaction.
- При изменении used cache list snapshot либо полностью фиксируется до stream,
  либо endpoint возвращает `active_list_cache_mismatch` fallback до первого
  active byte.
- Health/status публикуют generation identity и expected/actual hash, чтобы
  различать stale dnsmasq от generation failure.

### Тест-кейсы

1. Собрать snapshot с routed `work` и `cubly_home`, DNS rules для них и inline
   domains; nft backend генерирует `nftset=`, `server=/.../` и
   `rebind-domain-ok=` для обоих списков.
2. Добавить DNS-only `tailscale`: stream содержит `server=/ts.net/...` и
   rebind directive, но не содержит `nftset=` для `tailscale`.
3. Вычислить expected hash и сгенерировать stream из одного snapshot; MD5
   canonical directives и published hash идентичны.
4. Смоделировать разные `config_` и `ConfigStore` snapshots; generation не
   смешивает их и использует только published snapshot в `running`.
5. Во время `applying` resolver hook получает candidate, а параллельные
   status/CLI reads получают previous committed snapshot либо `busy`.
6. Удалить enabled list из snapshot или принудительно пропустить его stream;
   endpoint до первого byte возвращает structured error/fallback, а не header
   active config без list directives.
7. Dnsmasq fixture применяет stream; TXT hash совпадает, после чего runtime
   становится `running`.
8. Dnsmasq fixture применяет stream без ожидаемых `nftset=` entries либо
   публикует другой TXT hash; commit не происходит, выполняется rollback или
   runtime становится `broken`.
9. Изменить cache используемого list между snapshot capture и stream;
   endpoint возвращает `active_list_cache_mismatch` до первого active byte.
10. Regression fixture из OpenWrt: active config с `work`, `cubly_home` и
    `tailscale` никогда не выдаёт stream только из fallback/probe/TXT строк.
...
| 9. Resolver failures проглатываются | US-12, US-13, US-15, US-19 | Структурированные ошибки; fallback только до первого active byte; единый snapshot stream/hash/TXT |
...
4. **Transactions and source of truth:** US-10–US-14, US-19. Контрольная точка — daemon/CLI/dnsmasq/disk publish только согласованные snapshots.
...
- Каждый active resolver stream, expected hash и TXT confirmation происходят из одного immutable snapshot.
