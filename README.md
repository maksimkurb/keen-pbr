...
- **(опционально) Отключение автообновления списков:** `/opt/etc/cron.daily/50-keenetic-pbr-lists-update.sh`
   - Если Entware установлен на внутреннюю память, то настоятельно рекомендуется выключить автообновление списков, чтобы предотвратить износ NAND-памяти
...
Вы всегда сможете обновить списки вручную с помощью команды `keenetic-pbr download`.
